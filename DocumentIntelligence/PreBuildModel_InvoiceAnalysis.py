...
def main():
    ...
    try:
        ...
        fileUri = "https://microsoftlearning.github.io/mslearn-ai-information-extraction/Instructions/Exercises/media/sample-invoice.png"
        fileLocale = "en-US"
        ...
        for document in result.documents:
            ...
            billing_address = document.fields.get("ServiceAddress")
            if billing_address:
                print(f"Billing Address: {billing_address.content}, with confidence {billing_address.get('confidence')}.")
            
            billing_number = document.fields.get("InvoiceId")
            if billing_number:  
                print(f"Billing Number: {billing_number.get('valueString')}, with confidence {billing_number.get('confidence')}.")
            ...
            print("All Other fields from invoice document:")
            for field_name, field in document.fields.items():
                print(f"  {field_name}: {field.get('valueString') if field.get('valueString') else field.content}")
